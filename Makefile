...
# Every driver, and the date type `timestamp()` folds into. A test that names
# one is gated on its feature, so a narrower set silently runs fewer tests
# rather than failing.
FEATURES := postgres,sqlite,chrono
...
# Every driver alone, with and without dates, and then all of them together.
...
DRIVERS := postgres sqlite postgres,chrono sqlite,chrono
