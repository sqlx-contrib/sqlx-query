{
  description = "sqlx-query - splices SQL fragments into the sentinel comments of a query you already wrote.";
    ...
    flake-utils.lib.eachDefaultSystem (
      ...
      in
      {
        devShells.default = pkgs.mkShell {
          ...
          # the compose network.
          #
          # Silent when the stack is down, which is the point -- the database
          # tests read these and skip when they are unset, so `nix develop`
          # works with no Docker running.
          shellHook = ''
